...
REM Prepend MSVC, CMake, CUDA paths (MSVC first to ensure correct cl.exe/link.exe)
set "PATH=%MSVC_BIN%;%CMAKE_BIN%;%CUDA_BIN%;%PATH%"

REM Set CC/CXX for cmake
set "CC=cl.exe"
set "CXX=cl.exe"
REM Set CUDACXX so nvcc knows where to find cl.exe
set "CUDACXX=%MSVC_BIN%\cl.exe"
